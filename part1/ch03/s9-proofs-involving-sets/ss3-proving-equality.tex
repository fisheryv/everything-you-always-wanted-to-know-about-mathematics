...
纵观上述证明的整体结构，我们看到它分为两部分，因为它是一个双向包含证明。我们\emph{友好地提前}向读者指出了这一点，并将这两个部分明确分开。从技术上讲，忽略这一提示直接深入证明是可行的，但这可能会令读者感到困惑。证明的全部意义在于\emph{使他人信服}你所理解的事实，因此应尽可能让读者轻松理解你的思路。
...
\begin{proposition}
    \[\left\{x \in \mathbb{N} \mid x + \frac{8}{x} \le 6\right\} = \{2, 3, 4\}\]
\end{proposition}

\begin{proof}
    设 $A = \left\{x \in \mathbb{N} \mid x + \frac{8}{x} \le 6\right\}$, $B = \{2, 3, 4\}$。要证明 $A = B$，需要证明 $A \subseteq B$ 且 $B \subseteq A$。
