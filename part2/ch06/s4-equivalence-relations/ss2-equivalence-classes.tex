...
\begin{example}
    ...
    实际上，$\mathbb{R} / R$ 可与整数集 $\mathbb{Z}$ 建立对应。然而，我们通常写做 $\mathbb{R} / R ``='' \mathbb{Z}$ 是因为这种等式并不严谨。特别是，我们尚未形式化定义实数或整数，仅仅严格定义了自然数 $\mathbb{N}$。此处仅仅观察到等价类集与整数集存在对应关系，二者可以相互映射，但技术上并非\emph{相等}。
...
\begin{example}
    ...
    \[\left((x, y),(u, v)\right) \in R \iff x = u\]
...
\[\left((x, y),(u, v)\right) \in S \iff \sqrt{x^2+y^2} = \sqrt{u^2+v^2}\]
...
\begin{example}
    考虑集合 $S=[6]$。定义集合
    \[\mathcal{F} = \left\{ \{1, 4\}, \{2, 3, 5\} , \{6\} \right\}\]
